...
#endif
/* Network loop state */
enum net_loop_state net_state, net_state_ex;
...
int		NetTimeOffset;
#endif
int NetUdpListenPort = 0;/*udp listen port*/
...
static rxhandex_f *udp_packet_handler_ex=NULL;
...
static thand_f *timeHandler_ex;
/* Time base value */
static ulong	timeStart, timeStart_ex;
/* Current timeout value */
static ulong	timeDelta, timeDelta_ex;
/* THE transmit packet */
uchar *NetTxPacket;
...
restart:
	...
	for (;;) {
		...
		if(protocol == UDP)
		{
			if (timeHandler_ex && ((get_timer(0) - timeStart_ex) > timeDelta_ex)) {
				thand_f *x;
	...
	defined(STATUS_LED_RED)
		...
		if(protocol == UDP)
		{
			if (net_state_ex == NETLOOP_FAIL)
			{
				//NetStartAgain();
				net_set_udp_handler_ex(NULL);
				net_set_arp_handler(NULL);
				NetSetTimeout_ex(0, NULL);
				/* Invalidate the last protocol */
				eth_set_last_protocol(BOOTP);
...
void net_set_udp_handler_ex(rxhandex_f *f)
{
	debug_cond(DEBUG_INT_STATE, "--- NetLoop UDP handler set (%p)\n", f);
	udp_packet_handler_ex = f;
}
...
NetSetTimeout_ex(ulong iv, thand_f *f)
{
	if (iv == 0) {
		debug_cond(DEBUG_INT_STATE,
			"--- NetLoop timeout handler cancelled\n");
		...
		debug_cond(DEBUG_INT_STATE,
			"--- NetLoop timeout handler set (%p)\n", f);
		timeHandler_ex = f;
		timeStart_ex = get_timer(0);
		timeDelta_ex = iv * CONFIG_SYS_HZ / 1000;
	}
}
...
NetReceive(uchar *inpkt, int len)
{
	...
	case PROT_IP:
		...
		(*udp_packet_handler)((uchar *)ip + IP_UDP_HDR_SIZE,
				...
				ntohs(ip->udp_src),
				ntohs(ip->udp_len) - UDP_HDR_SIZE);
		if(udp_packet_handler_ex != NULL && ntohs(ip->udp_dst) == NetUdpListenPort)
		{
			(*udp_packet_handler_ex)((uchar *)ip + IP_UDP_HDR_SIZE,
					et->et_src,
					NetReadIP(&ip->ip_src),
					NetReadIP(&ip->ip_dst),
					ntohs(ip->udp_dst),
