...
typedef void rxhand_f(uchar *pkt, unsigned dport,
		      ...
		      unsigned len);
typedef void	rxhandex_f(uchar *, uchar *, IPaddr_t, IPaddr_t, unsigned, unsigned, unsigned);
...
extern int NetUdpListenPort;/*udp listen port*/
...
extern void	NetSetTimeout_ex(ulong iv, thand_f *f);
...
extern enum net_loop_state net_state,net_state_ex;
...
static inline int is_zero_ipaddr(const u8 *addr)
{
	return !(addr[0] | addr[1] | addr[2] | addr[3]);
}

static inline int is_valid_ipaddr(const u8 *addr)
{
    // 虽然不准确,但是排除尽可能多的无效地址. 如果无效地址,应该自动产生一个合法随机地址.
	return ((addr[0] > 0 && addr[0] < 224) && (addr[1] >= 0 && addr[1] < 255) && (addr[2] >= 0 && addr[2] < 255) && (addr[3] > 1 && addr[3] < 255));
}
