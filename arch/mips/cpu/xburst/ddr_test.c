...
/*#define BL_TEST*/
#ifdef BL_TEST
static void burst_test()
{
	unsigned int  tmp,flag = 0;
	unsigned int addrstart;
	addrstart = 0xa0000000;
	int i;
    debug("burst_chang_test run\n");
	for(i = 0;i < 32; i++){
		 if (test_ddr(addrstart,addrstart + i * 4,0,0))
			flag++;
	}

	if(flag)
           debug("burst_chang_test error!\nflag = %d\n",flag);
	else
	   debug("burst_chang_test ok!\n");
    debug("burst_chang_test over\n");
}
#endif

#ifdef DDR_REMAP_TEST
#define REMAP_TEST_ADDR	 0xa94cff0
static void remap_function_test(void)
{
	unsigned int remap1,remap2,remap3,remap4,remap5;
	set_remap(&remap1,&remap2,&remap3,&remap4,&remap5);
  {
    unsigned int tmp1,tmp2,remapa,remapb;
    unsigned int a,b;
    for(a = 0;a < 19;a++)
      for(b = a + 1;b < 20;b++){
          writel(0x1234567,REMAP_TEST_ADDR);
          debug("enter remap_swap\n");
          flush_cache_all();
          remapa = ddr_readl(DDRC_REMAP(a / 4 + 1));
          remapb = ddr_readl(DDRC_REMAP(b / 4 + 1));
          tmp1 = (remapa & (0x1f << ((a % 4) * 8))) >> ((a % 4) * 8);
          tmp2 = (remapb & (0x1f << ((b % 4) * 8))) >> ((b % 4) * 8);
          debug("before remap,tmp1 addr :%x,tmp1 = %d\n",&tmp1,tmp1);
          debug("before remap,tmp2 addr :%x,tmp2 = %d\n",&tmp2,tmp2);
          debug("before remap,remapa = %x	remapb = %x\n",ddr_readl(DDRC_REMAP(a / 4 + 1)),ddr_readl(DDRC_REMAP(b / 4 + 1)));
          debug("before remap ,the value of addr 0xa94cff0 is %x\n",readl(0xa94cff0));
	    if((a / 4) != (b / 4)){
		    remapa = (remapa & ~(0x1f << ((a % 4) * 8))) | (tmp2 << (a % 4) * 8);
		    remapb = (remapb & ~(0x1f << ((b % 4) * 8))) | (tmp1 << (b % 4) * 8);
		    ddr_writel(remapa,DDRC_REMAP(a / 4 + 1));
		    ddr_writel(remapb,DDRC_REMAP(b / 4 + 1));
	}else{
		remapa = (remapa & ~(0x1f << ((a % 4) * 8)) & ~(0x1f << (b % 4) * 8)) | (tmp2 << (a % 4) * 8) | (tmp1 << (b % 4) * 8);
		ddr_writel(remapa,DDRC_REMAP(a / 4 + 1));
	}
          debug("after remap,tmp1 addr :%x,tmp1 = %d\n",&tmp1,tmp1);
          debug("after remap ,the value of addr 0xa94cff0 is %x\n",readl(0xa94cff0));
          debug("after remap,tmp2 addr :%x,tmp2 = %d\n",&tmp2,tmp2);
          debug("after remap,remapa = %x	remapb = %x\n",ddr_readl(DDRC_REMAP(a / 4 + 1)), ddr_readl(DDRC_REMAP(b / 4 + 1)));
	unsigned int tag_addr;
	tag_addr = REMAP_TEST_ADDR & (0x1 << (tmp1 + 12));
	if(tag_addr){
		if(tag_addr = REMAP_TEST_ADDR & (0x1 << (tmp2 + 12)))
			tag_addr = REMAP_TEST_ADDR;
		else  tag_addr = (REMAP_TEST_ADDR & ~(0x1 << (tmp1 + 12))) | (0x1 << (tmp2 + 12));
	}
	else{
		if(tag_addr = REMAP_TEST_ADDR & (0x1 <<(tmp2 + 12)))
			tag_addr = (REMAP_TEST_ADDR & ~(0x1 << (tmp2 + 12))) | (0x1 << (tmp1 + 12));
		else tag_addr = REMAP_TEST_ADDR;
	}
	if(readl(tag_addr) != 0x1234567){
		debug("******************************\nfail! i = %d,j = %d,after remap addr = %x,value is %x\n",a,b,tag_addr,readl(tag_addr));
	}
        writel((3 & 0x1f) << 24 | (2 & 0x1f) << 16 | (1 & 0x1f) << 8 | (0 & 0x1f),
                DDRC_BASE + DDRC_REMAP(1));
        writel((7 & 0x1f) << 24 | (6 & 0x1f) << 16 | (5 & 0x1f) << 8 | (4 & 0x1f),
...
#endif
#ifdef POWERDOWN_TEST
static void powerdown_test(void)
{
	unsigned int ctrl_reg_value,state_reg_value;
    ctrl_reg_value = ddr_readl(DDRC_CTRL);
    state_reg_value = ddr_readl(DDRC_STATUS);
    /*ddr_writel(ddr_readl(DDRC_REFCNT) & (~(0x1)),DDRC_REFCNT);*/
	debug("ctrl_reg_value = %x\tstate_reg_value = %x\n",ctrl_reg_value,state_reg_value);
    debug("powerdown_test is running,please check CKE signal!\n");
    __asm__ __volatile__(
                      ...
                      ".set reorder\n\t"
                      : :"r"(0x1fffffff));
    #if 0 /*MDDR deep powerdown_test*/
    debug("MDDR deep powerdown_test is running,please check CKE signal!\n");
    __asm__ __volatile__(
                      ".set noreorder\n\t"
                      "1:\n\t"
                      "bne %0, 0, 1b\n\t"
                      "addi %0, %0, -1\n\t"
                      ".set reorder\n\t"
                      : :"r"(0x9ffff));     /*ensure all bank are in idle status*/
    ddr_writel(ddr_readl(DDRC_CTRL) | (0x1 << 6),DDRC_CTRL);
    /*while(!(ddr_readl(DDRC_STATUS) & 0x1 << 5));*/
    __asm__ __volatile__(
                      ".set noreorder\n\t"
                      "1:\n\t"
                      "bne %0, 0, 1b\n\t"
    ...
    ddr_writel(ddr_readl(DDRC_CTRL) & ~(0x1 << 6),DDRC_CTRL);
    #endif
    debug("powerdown_test over\n");
}
#endif

#ifdef SELF_REFRESH_TEST
static void self_refresh_funtion_test()
{
    unsigned int testaddr_st  = 0xa5000000,testaddr_end = 0xa50000ff;
    unsigned int i,value,ctrl_reg_value,flag = 0;
    debug("enter self_refresh_funtion_test\n");
    for (i = testaddr_st; i <= testaddr_end; i += 4)
        writel(i, i);
    flush_cache_all();
    ddr_writel(ddr_readl(DDRC_DLP) & 0,DDRC_DLP);
    ctrl_reg_value = ddr_readl(DDRC_CTRL);
    ctrl_reg_value |= 1 << 5;
    debug("DDR is going into self_refresh mode!\n");
    ddr_writel(ctrl_reg_value,DDRC_CTRL);
    while(!(ddr_readl(DDRC_STATUS2) & (0x1 << 13)));
    while(!(ddr_readl(DDRC_STATUS) & (0x1 << 2)));
    while(ddr_readl(DDRC_STATUS) & 0x3);    /*wait into  self_refresh mode*/
    /*warnning:  have entered  into self_refresh mode!
     * before mode exits, any  memory access is not allowed ,or system will be hang-up!
     */
    __asm__ __volatile__(
                      ".set noreorder\n\t"
                      "1:\n\t"
                      ...
                      ".set reorder\n\t"
                      : :"r"(0x1fffffff));
    ddr_writel(ddr_readl(DDRC_CTRL) & (~(1 << 5)),DDRC_CTRL);
    while(ddr_readl(DDRC_STATUS2) & (0x1 << 13));
    while(ddr_readl(DDRC_STATUS) & (0x1 << 2));
    while((ddr_readl(DDRC_STATUS) & 0x3) != 0x3);    /*wait  exit self_refresh mode*/

    debug("DDR has exited self_refresh mode\n");
    ddr_writel(ddr_readl(DDRC_DLP) & 0x3,DDRC_DLP);
    value = 0xffff;
    while(value--);
    ddr_writel(ddr_readl(DDRC_DLP) & 0x1,DDRC_DLP);
    value = 0xffff;
    while(value--);
    debug("testaddr_st = 0x%x\t testaddr_end = 0x%x\n",testaddr_st,testaddr_end);
	for (i = testaddr_st; i <= testaddr_end; i += 4) {
		value = readl(i);
		if (value != i) {
			debug("Error: the want value is 0x%x \n", i);
			debug("But the real value is 0x%x \n", value);
			debug("The address is 0x%x\n",i);
            flag++;
		}
	}
    if(flag)
        debug("the error num is %d\n",flag);
    debug("self_refresh_funtion test over\n");
}
#endif
