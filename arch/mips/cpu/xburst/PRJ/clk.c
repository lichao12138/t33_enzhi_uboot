...
struct cgu cgu_clk_sel_PRJ008[CGU_CNT] = {
	//          en cdr_off     cs_bit sel_src               sel[2]                   ce busy stop */
	[DDR]    = {1, CPM_DDRCDR,   30, CONFIG_DDR_SEL_PLL,    {0,    APLL, MPLL,  -1}, 29, 28, 27},
	[HELIX]  = {1, CPM_HELIXCDR, 30, MPLL,                  {APLL, MPLL, -1,    -1}, 29, 28, 27},
	[MACPHY] = {1, CPM_MACCDR,   30, CONFIG_MACPHY_SEL_PLL, {APLL, MPLL, -1,    -1}, 29, 28, 27},
	[LCD]    = {1, CPM_LPCDR,    30, APLL,                  {APLL, MPLL, -1,    -1}, 28, 27, 26},
	[MSC]    = {1, CPM_MSC0CDR,  30, CONFIG_CPU_SEL_PLL,    {APLL, MPLL, -1, EXCLK}, 29, 28, 27},
	[MSC1]   = {0, CPM_MSC1CDR,  30, CONFIG_CPU_SEL_PLL,    {APLL, MPLL, -1, EXCLK}, 29, 28, 27},
	[I2S]    = {1, CPM_I2STCDR,  30, APLL,                  {APLL, MPLL, -1,    -1}, 29, 28, 27},
	[SFC0]   = {1, CPM_SFC0CDR,  30, MPLL,                  {APLL, MPLL, -1,    -1}, 29, 28, 27},
	[SFC1]   = {1, CPM_SFC1CDR,  30, APLL,                  {APLL, MPLL, -1,    -1}, 29, 28, 27},
	[SSI]    = {1, CPM_SSICDR,   30, MPLL,                  {APLL, MPLL, -1,    -1}, 29, 28, 27},
	[PWM]    = {1, CPM_PWMCDR,   30, APLL,                  {APLL, MPLL, -1,    -1}, 29, 28, 27},
	[CIM]    = {1, CPM_CIMCDR,   30, APLL,                  {APLL, MPLL, -1,    -1}, 29, 28, 27},
	[ISP]    = {1, CPM_ISPMCDR,  30, MPLL,                  {APLL, MPLL, -1,    -1}, 29, 28, 27},
};

struct cgu cgu_clk_sel_PRJ007[CGU_CNT] = {
	[DDR]    = {1, CPM_DDRCDR,   30, CONFIG_DDR_SEL_PLL,    {0, APLL, MPLL, -1}, 29, 28, 27},
	[HELIX]  = {1, CPM_HELIXCDR, 30, MPLL,                  {APLL, MPLL, VPLL},  29, 28, 27},
	[MACPHY] = {1, CPM_MACCDR,   30, CONFIG_MACPHY_SEL_PLL, {APLL, MPLL, VPLL},  29, 28, 27},
	[LCD]    = {1, CPM_LPCDR,    30, VPLL,                  {APLL, MPLL, VPLL},  29, 28, 27},
	[MSC]    = {1, CPM_MSC0CDR,  30, CONFIG_CPU_SEL_PLL,    {APLL, MPLL, VPLL},  29, 28, 27},
	[MSC1]   = {1, CPM_MSC1CDR,  30, MPLL,                  {APLL, MPLL, VPLL},  29, 28, 27},
	[I2S]    = {1, CPM_I2STCDR,  30, VPLL,                  {APLL, MPLL, VPLL},  29, 28, 27}, //需要再配置; i2s 使用VPLL
	[SFC0]   = {1, CPM_SFC0CDR,  30, MPLL,                  {APLL, MPLL, VPLL},  29, 28, 27},
	[SFC1]   = {1, CPM_SFC1CDR,  30, VPLL,                  {APLL, MPLL, VPLL},  29, 28, 27},
	[SSI]    = {1, CPM_SSICDR,   30, MPLL,                  {APLL, MPLL, VPLL},  29, 28, 27},
	[PWM]    = {1, CPM_PWMCDR,   30, APLL,                  {APLL, MPLL, VPLL},  29, 28, 27},
	[CIM]    = {1, CPM_CIMCDR,   30, VPLL,                  {APLL, MPLL, VPLL},  29, 28, 27},
	[ISP]    = {1, CPM_ISPMCDR,  30, MPLL,                  {APLL, MPLL, VPLL},  29, 28, 27},
};

struct cgu *cgu_clk_sel = cgu_clk_sel_PRJ008;
...
void clk_prepare(void)
{
	...
	{
		// printf("PRJ007 clk prepare\n");
		cgu_clk_sel = cgu_clk_sel_PRJ007;
	}
...
#ifdef DUMP_CGU_SELECT
				printf("%s: 0x%X: value=0x%X j = %d  src = %d\n", cgu_name(i), reg, readl(reg), j, cgu_clks[i].sel_src);
#endif
				break;
			}
		}
...
static unsigned int get_mac_rate(unsigned int xcdr)
{
	...
	case 2:
		if(gd->arch.gi->soc.xburst.PRJ007)
			return pll_get_rate(VPLL) / ((maccdr & 0xff) + 1);
	default:
		break;
	}
...
void clk_set_rate(int clk, unsigned long rate)
{
	...
	if (gd->arch.gi->soc.xburst.PRJ007)
		cgu = &(cgu_clk_sel_PRJ007[clk]);
	else if (gd->arch.gi->soc.xburst.PRJ008)
		cgu = &(cgu_clk_sel_PRJ008[clk]);
	...
	while (cpm_inl(cgu->off) & (1 << cgu->busy))
		;
#ifdef DUMP_CGU_SELECT
	printf("%s(0x%x) :0x%x, pll_rate = %d, cgu->sel_src = %d\n",clk_name[clk] ,regval,  cpm_inl(cgu->off), pll_rate, cgu->sel_src);
#endif
	return;
}
...
void clk_init(void)
{
	...
	/*************设置外设的时钟源 31:30 ***************/
	cgu_clks_set(cgu_clk_sel, CGU_CNT);
}
