...
static void riscvhcdr_init(void)
{
	// while((cpm_inl(CPM_RISCVHCDR) & (0x1 << 28))); // 正常情况下这个时间点，riscv cpm busy应该为 0，所以不用等待 riscv busy
	if((cpm_inl(CPM_RISCVHCDR) & (0x1 << 28)))
		printf("riscv cpm busy!!!!\n"); //以防万一，加个打印；如果busy的话可能需要加上wait riscv busy动作
	cpm_outl(0x1 | (0x1 << 29), CPM_RISCVHCDR); // 配置riscv cpm默认2分频
}

int pll_init(void)
{
	if (gd->arch.gi->soc.xburst.PRJ008)
		riscvhcdr_init();

	// printf("%s:%d\n",__func__,__LINE__);
#ifdef CONFIG_PALLADIUM
	/**
	 * NOTE: in palladium, CCLK:L2CLK:H0CLK:H2CLK:PCLK:DDRCLK
	 * can be set as 1:1:1:1:1:1. each clk can be dived by APLL
	 * or MPLL.
	 **/
	{
		unsigned int cppcr = 0, cpapcr = 0, cpmpcr = 0, cpccr = 0, ddrcdr = 0;

		/* config CPPCR */
		cppcr = 0x00000020;
		cpm_outl(cppcr, CPM_CPPCR);
		while((cpm_inl(CPM_CPPCR) & (0x1 << 31)));

		/* config CPAPCR, not use in this version palladium
		   cpapcr = 0x01906409;
		   cpm_outl(cpapcr, CPM_CPAPCR);
		   while(!(cpm_inl(CPM_CPAPCR) & (0x1 << 3)));
		*/

		/* config CPMPCR, not use int this version palladium
		   cpmpcr = 0x01905109;
		   cpm_outl(cpmpcr, CPM_CPMPCR);
		   while(!(cpm_inl(CPM_CPMPCR) & (0x1 << 3)));
		*/
		...
		while(!cpm_inl(CPM_DDRCDR) & 0x30);
	}
#else /* CONFIG_PALLADIUM */
	freq_correcting();
#ifdef CONFIG_SPL_PARAMS_FIXER
	/* Switch SCLKA to EXCLK, and switch CPU/H0/H2 to SCLKA.
	 * 防止变频导致总线超频卡死
	 */
