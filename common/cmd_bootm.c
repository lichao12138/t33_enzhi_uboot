		...
		int boot_progress)
{
...
#ifdef CONFIG_GZIP
	case IH_COMP_GZIP:
		printf("   Uncompressing gzip %s ... ", type_name);
...
#ifdef CONFIG_BZIP2
	case IH_COMP_BZIP2:
		printf("   Uncompressing bzip2 %s ... ", type_name);
		/*
	...
	case IH_COMP_LZMA: {
		...
		printf("   Uncompressing lzma %s ... ", type_name);
/*lzma软件解压*/
		ret = lzmaBuffToBuffDecompress(load_buf, &lzma_len,
					       image_buf, image_len);
		unc_len = lzma_len;
		//printk("lzma unc_len = %d\n",unc_len);
		if (ret != SZ_OK) {
    ...
    case IH_COMP_HARD_LZMA:
    {
        printf("   Uncompressing LZMA Hardware %s ... ", type_name);
        /*lzma 硬件解压*/
        flush_cache_all();
        ret = jz_lzma_decompress(image_buf, image_len, load_buf);
        if (ret <= 0)
        {
            printf("lzam hardware decompress uImage failed!!! ret = %d\n", ret);
            bootstage_error(BOOTSTAGE_ID_DECOMP_IMAGE);
            return BOOTM_ERR_RESET;
        }
...
#ifdef CONFIG_USE_AES_SECURE_BOOT
	aes_data_len = get_aes_header_length_and_iv(img_addr, aes_iv);
	flush_cache_all();
	aes_efuse_init(aes_iv, CONFIG_AES_DEC_MODE, CONFIG_AES_EFUSE_KEY_BIT);
	aes_efuse_start(img_addr + 20, aes_data_len, img_addr);
	flush_cache_all();
#endif
