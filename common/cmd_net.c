...
int do_udp (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if (argc < 2) {
		NetUdpListenPort = simple_strtol(getenv("udplistenport"), NULL, 10);
		if (NetUdpListenPort == 0) {
			printf ("udplistenport not set, use default 10008\n");
			NetUdpListenPort = 10008;
		}
	} else {
		NetUdpListenPort = simple_strtol(argv[1], NULL, 10);
		if (NetUdpListenPort == 0) {
			printf ("Bad NetUdpListenPort, use default 10008\n");
			NetUdpListenPort = 10008;
		}
	}
	
	if (NetLoop(UDP) < 0) {
		printf("Udp: port %d listen exit\n", NetUdpListenPort);
		return 1;
	}
