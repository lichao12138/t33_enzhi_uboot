...
#define clrbits_le32(addr,clear)   		*(volatile unsigned int *)(addr) &= ~(clear)
#define setbits_le32(addr,set)     		*(volatile unsigned int *)(addr) |= (set)
#define clrsetbits_le32(addr,clear,set)		*(volatile unsigned int *)(addr) = ( (*(volatile unsigned int *)(addr))&(~(clear)) )  |(set)
...
DEFINE_ALIGN_BUFFER(uint8_t, aligned_buffer, DWC2_DATA_BUF_SIZE, 16);
DEFINE_ALIGN_BUFFER(uint8_t, status_buffer, DWC2_STATUS_BUF_SIZE, 16);

#define MAX_DEVICE			16
#define MAX_ENDPOINT			16
static int bulk_data_toggle[MAX_DEVICE][MAX_ENDPOINT];
static int inbulk_data_toggle[MAX_DEVICE][MAX_ENDPOINT];
static int outbulk_data_toggle[MAX_DEVICE][MAX_ENDPOINT];
...
static void dwc_otg_core_host_init(struct dwc2_core_regs *regs)
{
	...
	clrbits_le32(&regs->gotgctl, DWC2_GOTGCTL_HSTSETHNPEN);
        ...
        /*zm*/
       /* setbits_le32(&regs->gotgctl,DWC2_GOTGCTL_OTGVER);*/
	setbits_le32(&regs->gotgctl,0x0c);
...
static void dwc_otg_core_init(struct dwc2_core_regs *regs)
{
	...
	usbcfg |= DWC2_GUSBCFG_FORCEHOSTMODE;/*zm*/
		    ...
		    int len)
{
	...
	if (usb_pipein(pipe))
		pid = &inbulk_data_toggle[devnum][ep];
	else
		pid = &outbulk_data_toggle[devnum][ep];

	return chunk_msg(dev, pipe, pid,
	...
	clrbits_le32(&regs->hprt0, DWC2_HPRT0_PRTENA | DWC2_HPRT0_PRTCONNDET |
        ...
        for (i = 0; i < MAX_DEVICE; i++) {
                for (j = 0; j < MAX_ENDPOINT; j++){
                    inbulk_data_toggle[i][j] = DWC2_HC_PID_DATA0;
                    outbulk_data_toggle[i][j] = DWC2_HC_PID_DATA0;
                }
        }
	return 0;
}
