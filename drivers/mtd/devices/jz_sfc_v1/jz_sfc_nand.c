...
int sfc_do_nand_features(unsigned char feature_res, unsigned char feature_dir, unsigned char *feature_val)
{
	unsigned char tmp = 0;
	if(feature_res){
		sfc_nand_reset();
	...
	if(feature_dir)	return sfc_nand_set_feature(flash,feature_val[0],feature_val[1]);

	if(!sfc_nand_get_feature(flash,feature_val[0],&tmp))printf("0x%02x\n", tmp&0xff);
